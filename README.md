...
To diagnose fixture generation without exposing the API key, set `YANDEX_SPEECHKIT_DEBUG=1` for the command. It prints the endpoint, non-secret form fields, a redacted authorization header, response status and headers, and a redacted response body capped at 16 KiB for non-200 responses.
