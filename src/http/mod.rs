...
<body>
  ...
  <script>
    ...
    $('connect').addEventListener('click', async () => { token = $('token').value; if (!token) { setMessage('message', 'Введите токен.', true); return; } const response = await api('/v1/search', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: 'rock', limit: 1 }) }); if (!response.ok) { setMessage('message', 'Сервер отклонил токен.', true); token = ''; return; } $('login').style.display = 'none'; $('workspace').style.display = 'block'; await readiness(); runSearch(); });
