...
An ignored `yandex_speechkit_live` integration test now covers a real, pre-recorded mono Ogg/Opus command against SpeechKit's synchronous endpoint. Its committed fixture and expected phrase are generated explicitly by `generate_speechkit_fixture` through Yandex TTS; both network operations are billable and only run when selected. The recognition test is gated by explicit credentials, while optional variables can override the committed audio and phrase. Safe tracing records status, elapsed time, response size category, and a boolean match result without logging credentials, audio, or transcript. It does not make the streaming `SpeechStreamSession` production-ready.
