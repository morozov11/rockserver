...
An ignored `yandex_speechkit_live` integration test now covers a real, pre-recorded mono Ogg/Opus command against SpeechKit's synchronous endpoint. It is gated by explicit environment variables for the API key, folder, local audio path, and expected phrase; it makes a billable external request only when explicitly selected. Safe tracing records status, elapsed time, response size category, and a boolean match result without logging credentials, audio, or transcript. It does not make the streaming `SpeechStreamSession` production-ready and has not been live-run in this workspace.
