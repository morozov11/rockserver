...
|---|---|---|
| Canonical identity | RM-004 задаёт глобально уникальный, не зависящий от URL `station.id`; IDs не переиспользуются. Изменение URL operational-successor потока сохраняет station/stream ID; rename сохраняет station ID. См. [rm-004-contract-proposal.md](rm-004-contract-proposal.md). | Персистентная ссылка на станцию — только canonical `stationId`, никогда URL, имя, alias или stream ID. |
