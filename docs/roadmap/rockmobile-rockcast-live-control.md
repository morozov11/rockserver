...
**Статус:** RS-8/RS-9, RC-4/RC-4b и RM-4 реализованы. 2026-09-26 два
переключения станций с подключённого телефона на обновлённый RockCast прошли
через развёрнутый RockServer; имя станции и трек отобразились. Полная
физическая матрица, включая голос с микрофона и восстановление после offline,
остаётся открытой.
...
**Последовательность выпуска:** совместимый RockCast и RockServer обновлены;
RockMobile установлен на телефон. Частичная физическая приёмка пройдена,
оставшиеся сценарии перечислены в Phase 4.
...
«Что играет» означает подтверждённую станцию: `station_id` из state сопоставляется с
точной записью каталога RockServer и, при его недоступности, с проверенным локальным
extended SQLite. Необязательный `track_title` из ICY-метаданных RockCast показывается
отдельно, когда поток его передаёт; это поле не меняет идентичность станции.
Server-to-target delivery также несёт bounded display name, чтобы RockCast
не показывал UUID, если у него нет локальной карточки. Если запись недоступна
и на сервере, и в локальном каталоге Mobile, UI
